...
1. SHOULD: External resource fetching (fonts, assets) in API routes SHOULD use fetch() with absolute URLs constructed via import.meta.url for reliability

## Policy Block

- SHOULD External resource fetching (fonts, assets) in API routes SHOULD use fetch() with absolute URLs constructed via import.meta.url for reliability
