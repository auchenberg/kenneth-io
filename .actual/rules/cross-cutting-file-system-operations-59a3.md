...
```bash
# Detect unsafe JSON.parse patterns without error handling in file-reading code
...
<enforcement>
Claude Code MUST NOT skip or defer verification. ESLint rules detecting unsafe JSON.parse patterns without error handling are mandatory. Code review must block merge until validation patterns are implemented. Security team must be notified of dependency vulnerabilities for triage and remediation.
</enforcement>