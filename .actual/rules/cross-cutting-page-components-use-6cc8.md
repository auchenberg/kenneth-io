...
- **R-NEXTJS-006** SHOULD: Data fetching logic SHOULD be extracted into separate helper modules to maintain separation of concerns.
- **R-NEXTJS-007** SHOULD: API routes requiring low-latency or global distribution SHOULD specify `runtime='edge'` in route config.
- **R-NEXTJS-008** SHOULD: Internal navigation SHOULD use `next/link` to enable client-side transitions.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All rules in this file are mandatory for page components and API routes within scope. Violations must be flagged during code review and CI pipeline checks must fail if Next.js conventions are not detected.
</enforcement>