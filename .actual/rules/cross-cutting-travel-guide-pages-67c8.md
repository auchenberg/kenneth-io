...
These rules are ALWAYS ACTIVE for all files under `pages/travel/` with dynamic routing, list pages at `pages/lists.js`, and any pages sourcing content from `helpers/getTravelGuides` or `helpers/getBucketList`.
...
- **R-TG-006** MUST: All pages using static generation MUST import React and Layout components from `components/layout` for consistent UI structure.
- **R-TG-007** SHOULD: Use `next/link` for navigation between statically generated pages to enable client-side transitions without full page reloads.
- **R-TG-008** MAY: Consider Incremental Static Regeneration (ISR) with `revalidate` option if content update frequency increases while maintaining static generation benefits.
