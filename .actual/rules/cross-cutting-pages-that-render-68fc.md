...
- **R-PAGES-005** SHOULD: Page components SHOULD import and wrap content with the Layout component from `components/layout` for consistent structure.
...
- Data fetching helper functions are located in `helpers/` directory and imported by page components
- Page components wrap content with Layout component from `components/layout`

<enforcement>
Claude Code MUST NOT skip or defer verification. All rules in this file are mandatory for content-driven pages. Violations must be flagged during code review and build-time verification.
</enforcement>