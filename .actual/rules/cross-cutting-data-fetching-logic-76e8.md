...
```bash
# Verify getStaticProps exports exist in page components
grep -r 'export.*getStaticProps' pages/ --include='*.js' --include='*.jsx' --include='*.ts' --include='*.tsx'

# Verify getStaticPaths exports exist in dynamic route pages
...
- Data fetching logic is extracted into dedicated helper functions rather than inlined in `getStaticProps`
