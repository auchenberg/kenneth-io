...
These rules are ALWAYS ACTIVE for Next.js API routes in the `pages/api/` directory using edge runtime serverless functions with public-facing HTTP GET endpoints and query parameter-based request interfaces.
...
- Parameter validation is implemented immediately after `searchParams.get()` calls
- Null/undefined checks handle missing parameters gracefully

<enforcement>
Clause Code MUST NOT skip or defer verification of these rules during code review and automated linting checks.
</enforcement>