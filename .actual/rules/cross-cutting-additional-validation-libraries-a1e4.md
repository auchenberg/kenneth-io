...
- **R-VALIDATION-001** MAY: Additional validation libraries MAY be used for complex input validation scenarios beyond basic JSON parsing.
- **R-VALIDATION-002** MUST: All JSON.parse() operations on file contents MUST be wrapped in try-catch blocks with appropriate error handling.
- **R-VALIDATION-003** MUST: File system operations MUST consistently use Node.js core 'fs' and 'path' modules.
- **R-VALIDATION-004** MUST: Public API functions MUST validate input paths and sanitize error messages before exposing to consumers.
- **R-VALIDATION-005** MUST: No high or critical severity vulnerabilities MUST exist in gray-matter or lodash dependencies.
...
<enforcement>
Verification via ESLint rules detecting unsafe JSON.parse patterns, code review checklists for file system operations, automated security scanning in CI/CD pipeline, and integration tests validating error handling is mandatory. Claude Code MUST NOT skip or defer verification.
</enforcement>