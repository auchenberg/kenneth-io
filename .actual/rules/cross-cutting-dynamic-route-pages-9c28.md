...
```bash
# Verify getStaticProps exports in page components
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Build failures block deployment if `getStaticProps` functions fail during static generation. Code review must verify that new pages follow `getStaticProps`/`getStaticPaths` patterns before merge approval.
</enforcement>