...
- **R-HELPER-003** MUST: Use `gray-matter` for parsing markdown files with frontmatter, extracting metadata into a data object and content into a separate field.
...
- **R-HELPER-005** SHOULD: Use Node.js filesystem operations (`fs`, `path`) and `markdown-it` for content transformation in helper modules.
...
- **R-HELPER-008** SHOULD: Maintain clear separation between helper modules (framework-agnostic) and Next.js-specific page functions to isolate framework dependencies.
